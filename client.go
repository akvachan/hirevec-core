...
func (c Client) Register(ctx context.Context, email, fullName, password string) (*TokenPair, error) {
	...
	var pair TokenPair
	err := c.DoRequest(ctx, http.MethodPost, string(RouteV1Me), payload, false, &pair)
	return &pair, err
}
...
func (c Client) Login(ctx context.Context, email, password string) (*TokenPair, error) {
	...
	var pair TokenPair
	err := c.DoRequest(ctx, http.MethodPost, string(RouteOAuth2Authorize)+"?provider=email", payload, false, &pair)
	return &pair, err
}
...
func (c Client) GetMeRecommendations(ctx context.Context, posCursor, canCursor string, limit int, excludeReacted bool) (*JSONAPIDocument, error) {
	var doc JSONAPIDocument
