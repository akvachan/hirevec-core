...
func main() {
	...
	if err := hirevec.RunApp(hirevec.AppConfig{
		...
		PostgreSQLDatabaseURL:       hirevec.GetenvAndParse("HIREVEC_POSTGRESQL_DATABASE_URL", url.Parse, &url.URL{Host: ""}),
		TEIBaseURL:                  hirevec.GetenvAndParse("HIREVEC_TEI_BASE_URL", url.Parse, &url.URL{Host: ""}),
		TEIAPIKey:                   os.Getenv("HIREVEC_TEI_API_KEY"),
		SymmetricKey:                os.Getenv("HIREVEC_SYMMETRIC_KEY"),
		AsymmetricKey:               os.Getenv("HIREVEC_ASYMMETRIC_KEY"),
		GoogleClientID:              os.Getenv("HIREVEC_GOOGLE_CLIENT_ID"),
